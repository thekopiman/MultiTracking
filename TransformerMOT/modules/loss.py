...
class MotLoss(nn.Module):
    def __init__(self, params):
        ...
        if params.loss.type == "gospa":
            check_gospa_parameters(
                params.loss.cutoff_distance, params.loss.order, params.loss.alpha
            )
    ...
    def compute_hungarian_matching(
        self, predicted_states, predicted_logits, targets, distance="detr", scaling=1
    ):
        ...
        permutation_idx = [
            (
                torch.as_tensor(i, dtype=torch.int64).to(self.device),
                torch.as_tensor(j, dtype=torch.int64).to(self.device),
            )
            for i, j in indices
        ]
    ...
    def compute_orig_gospa_matching(self, outputs, targets, existence_threshold):
        ...
        assert (
            self.alpha == 2
        ), "The permutation -> assignment relation used to decompose GOSPA might require that loss.alpha == 2"
        ...
        for i in range(bs):
            alive_idx = (
                output_existence_probabilities[i, :].squeeze(-1) > existence_threshold
            )
            ...
            if len(current_targets) == 0:
                ...
                indices.append(([], []))
                loss += torch.Tensor(
                    [self.miss_cost / self.alpha * len(current_targets)]
                )
                ...
                missed_target_cost += (len(current_targets) - permutation_length) * (
                    self.miss_cost / self.alpha
                )
                false_target_cost += (len(alive_output) - permutation_length) * (
                    self.miss_cost / self.alpha
                )

        decomposition = {
            "localization": localization_cost,
            "missed": missed_target_cost,
            "false": false_target_cost,
            "n_matched_objs": permutation_length,
        }
        return loss, indices, decomposition

    def compute_orig_gospa_matching_with_uncertainties(
        ...
        assert (
            batch_size == 1
        ), "GOSPA matching with uncertainties currently only works with batch size = 1"
        ...
        predicted_distributions = {
            "states": predictions["state"][0, alive_idx].detach(),
            "state_covariances": predictions["state_covariances"][
                0, alive_idx
            ].detach(),
        }
        targets = targets[0]
        ...
        if n_targets == 0:
            ...
            missed_target_cost += (n_targets - permutation_length) * (
                self.miss_cost / self.alpha
            )
            false_target_cost += (n_predictions - permutation_length) * (
                self.miss_cost / self.alpha
            )
    ...
    def gospa_forward(
        self, outputs, targets, probabilistic=True, existence_threshold=0.75
    ):
        ...
        if probabilistic:
            indices, cost_matrix, unmatched_x = self.compute_prob_gospa_matching(
                outputs, targets
            )
            ...
            for i in range(bs):
                ...
                batch_cost = (
                    batch_cost
                    + output_logits[i][unmatched_x[i]].sum() * self.miss_cost / 2.0
                )
            ...
            assert (
                0 <= existence_threshold < 1
            ), "'existence_threshold' should be in range (0,1)"
            loss, indices, decomposition = self.compute_orig_gospa_matching(
                outputs, targets, existence_threshold
            )
            loss = loss / bs
            return loss, indices, decomposition
    ...
    def state_loss(self, predicted_states, targets, indices, uncertainties=None):
        ...
        if uncertainties is not None:
            ...
            prediction_distribution = torch.distributions.normal.Normal(
                matched_predicted_states, matched_uncertainties
            )
            loss = -prediction_distribution.log_prob(target).mean()
        else:
            loss = (
                F.l1_loss(matched_predicted_states, target, reduction="none")
                .sum(-1)
                .mean()
            )
    ...
    def logits_loss(self, predicted_logits, targets, indices):
        ...
        target_classes = torch.zeros_like(
            predicted_logits, device=predicted_logits.device
        )
        target_classes[idx] = 1.0  # this is representation of an object
        loss = F.binary_cross_entropy_with_logits(
            predicted_logits.squeeze(-1).permute(1, 0),
            target_classes.squeeze(-1).permute(1, 0),
        )
    ...
    def get_loss(self, prediction, targets, loss_type, existence_threshold=None):
        ...
        if self.params.data_generation.prediction_target == "position":
            ...
            predicted_states = torch.cat(
                (prediction.positions, prediction.velocities), dim=2
            )
        else:
            raise NotImplementedError(
                f"Hungarian matching not implemented for prediction target "
        ...
        if loss_type == "gospa":
            ...
            indices, _ = self.compute_hungarian_matching(
                predicted_states, prediction.logits, targets
            )
    ...
    def forward(
        ...
        encoder_prediction=None,
        loss_type="detr",
        existence_threshold=0.75,
    ):
        if loss_type not in ["gospa", "gospa_eval", "detr"]:
            raise NotImplementedError(
                f"The loss type '{loss_type}' was not implemented.'"
            )

        # print("I ran")
        losses = {}
        loss, indices = self.get_loss(
            prediction, targets, loss_type, existence_threshold
        )
        losses.update(loss)

        if intermediate_predictions is not None:
            for i, intermediate_prediction in enumerate(intermediate_predictions):
                aux_loss, _ = self.get_loss(
                    intermediate_prediction, targets, loss_type, existence_threshold
                )
                aux_loss = {f"{k}_{i}": v for k, v in aux_loss.items()}
                losses.update(aux_loss)

        if encoder_prediction is not None:
            enc_loss, _ = self.get_loss(
                encoder_prediction, targets, loss_type, existence_threshold
            )
