...
from TransformerMOT.util.plotting import (
    output_truth_plot,
    compute_avg_certainty,
    get_constrastive_ax,
    get_false_ax,
    get_total_loss_ax,
...
from TransformerMOT.modules.loss import MotLoss
from TransformerMOT.modules.contrastive_loss import ContrastiveLoss
from TransformerMOT.modules import evaluator
from TransformerMOT.models.BOMTv2 import BOMT
from simulation.simulations.data_generator import (
    DataGenerator,
...
if __name__ == "__main__":
    ...
    logger = Logger(
        log_path=f"{results_folder_path}/{exp_name}",
        save_output=False,
    ...
    if not (params.training.batch_size % n_splits == 0):
        raise ValueError(
            "'params.training.batch_size' must be divdeble with 'params.training.n_splits'"
        )
    ...
    if args.continue_training_from is not None:
        ...
        idx_last = np.argmax(
            [int(re.findall(r"\d+", c)[-1]) for c in checkpoint_names]
        )  # extract last occurrence of a number from the names
    ...
    for i_gradient_step in range(params.training.n_gradient_steps):
        ...
        try:
            ...
            batch, labels, unique_ids, target_coordinates = data_generator.get_batch()
            (
                prediction,
            ...
            logs = update_logs(
                logs,
                "last_layer_losses",
                loss_dict[f"{params.loss.type}_logits"].item()
                + loss_dict[f"{params.loss.type}_state"].item(),
            )

            if params.loss.contrastive_classifier:
                c_loss = contrastive_loss(
                    aux_classifications["contrastive_classifications"], optim_indices
                )
            ...
            matched_quants, unmatched_quants = compute_avg_certainty(
                outputs_history, indices_history
            )
            ...
            if (
                ...
                and params.training.max_gradnorm > 0
            ):
                torch.nn.utils.clip_grad_norm_(
                    model.parameters(), params.training.max_gradnorm
                )
        ...
        matched_median_certainties.append(
            np.mean(np.array(logs["matched_median_certainty"]))
        )
        ...
        unmatched_min_certainties.append(
            np.mean(np.array(logs["unmatched_min_certainty"]))
        )
        unmatched_q1_certainties.append(
            np.mean(np.array(logs["unmatched_q1_certainty"]))
        )
        unmatched_median_certainties.append(
            np.mean(np.array(logs["unmatched_median_certainty"]))
        )
        unmatched_q3_certainties.append(
            np.mean(np.array(logs["unmatched_q3_certainty"]))
        )
        unmatched_max_certainties.append(
            np.mean(np.array(logs["unmatched_max_certainty"]))
        )
        ...
        try:
            if (
                ...
                and i_gradient_step % params.debug.plot_interval == 0
            ) or (
                params.debug.save_plot_figs
                and i_gradient_step % params.debug.save_plot_figs_interval == 0
            ):
                ...
                unmatched_median_cert_line.set_data(
                    x_axis, np.array(unmatched_median_certainties)
                )
                percent_ax.fill_between(
                    x_axis,
                    unmatched_min_certainties,
                ...
                for line, data in zip(
                    ...
                    [avg_stds_x, avg_stds_y, avg_stds_vx, avg_stds_vy],
                ):
        ...
        for k, v in logs.items():
            logger.log_scalar(
                os.path.join("metrics", k), np.mean(np.array(v)), i_gradient_step
            )
        ...
        if new_lr != current_lr:
            current_lr = new_lr
            logger.log_scalar(
                "metrics/learning_rate", current_lr, i_gradient_step, flush_now=True
            )
        ...
        if (
            params.debug.evaluate_gospa_interval is not None
            and (i_gradient_step + 1) % params.debug.evaluate_gospa_interval == 0
        ):
            data_generator_eval = DataGenerator(eval_params)
            print("Starting periodic evaluation...")
            gospa_results = evaluator.evaluate_gospa(
                data_generator_eval, model, eval_params
            )
            logger.log_scalar(
                "metrics/gospa_total", gospa_results[0], i_gradient_step, flush_now=True
            )
            logger.log_scalar(
                "metrics/gospa_localization_error",
                gospa_results[1],
                i_gradient_step,
                flush_now=True,
            )
            logger.log_scalar(
                "metrics/gospa_localization_error_normalized",
                gospa_results[2],
                i_gradient_step,
                flush_now=True,
            )
            logger.log_scalar(
                "metrics/gospa_missed",
                gospa_results[3],
                i_gradient_step,
                flush_now=True,
            )
            logger.log_scalar(
                "metrics/gospa_false", gospa_results[4], i_gradient_step, flush_now=True
            )
