...
class DataGenerator:
    def __init__(self, params, rngs=None, simulation_generator=None):
        self.params = params
        assert (
            0
            <= params.data_generation.n_prediction_lag
            <= params.data_generation.n_timesteps
        ), "Prediction lag has to be smaller than the total number of time-steps."
        ...
        if rngs is None:
            ...
            ), "The number of provided RNGs must matchs the desired batch size"
        self.datagens = [simulation_generator(params, rng=rng) for rng in rngs]
...
def get_single_training_example(data_generator, n_timesteps):
    ...
    while len(label_data) == 0 or len(data_generator.measurements) == 0:
        ...
        for i in range(n_timesteps - 1):
            data_generator.step()
        training_data, label_data, unique_measurement_ids, unique_label_ids = (
